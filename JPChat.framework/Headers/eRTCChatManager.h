...
-(void)CreatePrivateGroup:(NSMutableDictionary *)dictParam withGroupImage:(NSData *)ProfilePicData andCompletion:(chatApiCompletion)success andFailure:(chatApiFailure) failure;
...
-(void)getHistory:(NSString*)searchText limit:(NSUInteger)limit skip:(NSUInteger) skip threadID: (NSString* _Nullable) threadID completion:(blockApiSuccess)success andFailure:(blockApiFailure) failure;
@end
